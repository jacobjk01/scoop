...
const TourBooking1 = ({navigation}) => {
  ...
  const [times, setTimes] = useState({
    timeRanges: ["8:00 AM - 12:00 PM", "12:00 PM - 5:00 PM","5:00 PM - 10:00 PM",["- - - -", ""]],
  ...
  const checkDate = () => {
    let text
    if((times.selectedTimes[0] == true || times.selectedTimes[1] == true || times.selectedTimes[2] == true || times.selectedTimes[3] == true) && selectedDate == '') {
      text = "Please select a date"
    }
    else if((times.selectedTimes[0] == false && times.selectedTimes[1] == false && times.selectedTimes[2] == false && times.selectedTimes[3] == false) && selectedDate != '') {
    ...
    if (customStartTime != "Please Select" && startOrEnd == true) {
      return(
        <DatePicker
          date={date}
          mode={'time'}
          style={{height: 160, marginLeft: 'auto', marginRight: 'auto', width: 180}}
  ...
  const confirm = () => {
    if (customStartTime != "Please Select" && customEndTime != "Please Select") {
      return(
        ...
        >
          <Text style={{fontSize: 14, color: colors.white, backgroundColor: colors.primary, borderRadius: 8, paddingHorizontal: 14, paddingVertical: 6, marginVertical: 3}}>
            Confirm
          </Text>
        </TouchableOpacity>
      )
      ...
      return(
        <TouchableOpacity
        >
          <Text style={{fontSize: 14, color: colors.grayDark, borderWidth: 1, borderColor: colors.grayDark, borderRadius: 8, paddingHorizontal: 14, paddingVertical: 5, marginVertical: 4}}>
            Confirm
          </Text>
        </TouchableOpacity>
      )
  ...
  const checkStart = () => {
    if (customStartTime == "Please Select") {
      ...
      return moment(customStartTime).format('LT')
  }
  const checkEnd = () => {
    if (customEndTime == "Please Select") {
      ...
      return moment(customEndTime).format('LT')
  }

  const checkIfCustom = (index) => {
    if (index == 3 && times.timeRanges[3][0] != "- - - -")
      return moment(times.timeRanges[3][0]).format('LT') + ' - ' + moment(times.timeRanges[3][1]).format('LT')
    else
      return times.timeRanges[index]
  }
        ...
        <View style={{marginTop: 85, marginRight: 12, marginLeft: 12,}}>
          ...
          <View style={[styles.backCard, {paddingBottom: 15}]}>
            ...
            <Calendar
              ...
              renderArrow={(direction) => {
                if(direction == 'left') {
                  return (
                    <View style={{borderWidth: 1, borderRadius: 20, borderColor: colors.grayLight}}>
                      <Ionicons name='chevron-back-outline' size={11} color={colors.primary} style={{padding: 4}}/>
                    </View>
                  )
                } else {
                  return(
                    <View style={{borderWidth: 1, borderRadius: 20, borderColor: colors.grayLight}}>
                     <Ionicons name='chevron-forward-outline' size={11} color={colors.primary} style={{padding: 4}}/>
                    </View>
                  )
                }
          ...
          >
            <View style={{backgroundColor: 'rgba(0,0,0,0.5)', width: '100%', height: '100%'}}>
              <View style={{backgroundColor: colors.white, height: '42.5%', width: '53%', marginTop: 'auto', marginBottom: 'auto', marginRight: 'auto', marginLeft: 'auto', borderRadius: 10}}>
                <TouchableWithoutFeedback onPress={() => {
                    setStartOrEnd(!startOrEnd)
                    animation()
                  }}>
                  <View style={{backgroundColor: '#F2f2f2', display: 'flex', flexDirection: 'row', marginLeft: 'auto', marginRight: 'auto', marginTop: 15, borderRadius: 15}}>
                    <View style={{paddingLeft: 15, paddingRight: 10, paddingTop: 10, paddingBottom: 10}}>
                      <Text style={{fontSize: 14, fontWeight: '700', color: colors.black}}>
                        Start
                      </Text>
                      <Text style={{fontSize: 10, color: colors.black, width: 61}}>
                        {checkStart()}
                      </Text>
                    </View>
                    ...
                    <Animated.View style={{backgroundColor: colors.white, position: 'absolute', top: '6%', left: slideAnim, height: '88%', width: 88, zIndex: -1, borderRadius: 13, }}>

                    </Animated.View>
                  </View>
                </TouchableWithoutFeedback>
                {displayTimePicker()}
                <View style={{display: 'flex', flexDirection: 'row', width: '86%',marginRight: '7%', marginLeft: '7%', borderRadius: 10, justifyContent: 'space-evenly'}}>
                  <TouchableOpacity 
                    onPress={() => {
                      setModalVisible(false)
                      let timeRanges = [...times.timeRanges]
                    ...
                    style={{fontSize: 14, color: colors.white, backgroundColor: colors.primary, borderRadius: 8, paddingHorizontal: 14, paddingVertical: 6, marginVertical: 3}}
                    >
          ...
          <View style={styles.timeView}>
            {times.selectedTimes.map((date, index) => {
              ...
              let backColor, textColor, subTextColor
                ...
                return(
                    <TouchableOpacity 
                      style={{backgroundColor: backColor, padding: 8, marginRight: 9, marginLeft: 9, marginBottom: 4, marginTop: 5, width: 170, elevation: 10, borderRadius: 5}}
                      onPress={() => {
                        updateSelectedTime(index)
                        viewModal(index)
                    }}>
                      <Text style={{color: textColor, fontSize: 16.5, fontFamily: 'Helvetica', marginLeft: 'auto', marginRight: 'auto', fontWeight: '700'}}>
                        {range[index]}
                      </Text>
                      <Text style={{color: subTextColor, alignSelf: 'center', fontSize: 12, fontWeight: '400'}}>
                        {checkIfCustom(index)}
                      </Text>
                    </TouchableOpacity>
                )
